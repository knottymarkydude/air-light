...
if ( ! function_exists( 'air_light_entry_footer' ) ) {
	...
	function air_light_entry_footer() {
		if ( 'post' === get_post_type() ) {
			$categories_list = get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma', 'air-kmd' ) );
			if ( $categories_list ) : ?>
				<p class="cat"><?php echo get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'air-kmd' ) ); ?></p>
			<?php	endif;

			$tags_list = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'air-kmd' ) );
			if ( $tags_list ) {
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			comments_popup_link( sprintf( wp_kses( __( 'Leave a comment<span class="screen-reader-text"> on %s</span>', 'air-kmd' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
			echo '</span>';
		}

		edit_post_link(
			sprintf( _x( 'Edit %s', '%s: Name of current post', 'air-kmd' ), the_title( '<span class="screen-reader-text">"', '"</span>', false ) ),
			'<p class="edit-link">',
			'</p>'
		);
	}
		...
		<li id="li-comment-<?php comment_ID(); ?>" <?php comment_class(); ?>>
			<div id="comment-<?php comment_ID(); ?>">
				...
				<?php if ( '0' === $comment->comment_approved ) : ?>
					<p><em><?php esc_html_e( 'Your comment is awaiting approval.', 'air-kmd' ); ?></em></p>
				<?php endif; ?>
				...
				<p class="comment-time">
					<a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ); ?>">
						...
						<time datetime="<?php echo get_comment_date( 'c' ); ?>"><?php printf( __( '%1$s at %2$s', 'air-kmd' ), get_comment_date(), get_comment_time() ); ?></time>
					</a>
				</p>
				...
				edit_comment_link( __( '&mdash; Edit', 'air-kmd' ), ' ', '' ) ?>
